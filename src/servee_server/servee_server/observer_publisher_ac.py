...
class Server:
    def __init__(self, host, port, db_manager):
        ...
        self.CERTIFILE="./src/etc/certificates/server.crt"
        self.KEYFILE="./src/etc/certificates/server.key"

        self.tls_context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        self.tls_context.minimum_version = ssl.TLSVersion.TLSv1_2  
        self.tls_context.load_cert_chain(certfile=self.CERTIFILE, keyfile=self.KEYFILE)
    ...
    def start(self):
        ...
        with self.tls_context.wrap_socket(self.server_socket, server_side=True) as tls_socket:
            while True:
                print("Waiting for a secure connection...")
                client_socket, addr = tls_socket.accept()
                print(f"Secure connection established with {addr}")

                print(f"Cipher Suite: {client_socket.cipher()}") # 서버에서 사용된 암호 스위트 확인
